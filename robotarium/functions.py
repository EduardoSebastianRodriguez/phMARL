...
class LEMURS_actor(nn.Module):
    ...
    def forward(self, x, laplacian_base):
        ...
        if self.scenario_name != "navigation_robotarium":
            batch_size = x.shape[0]
        else:
            batch_size = 1
        ...
        if self.scenario_name == "navigation_robotarium":
            select_states = torch.zeros(self.na, self.observation_dim_per_agent, device=self.device)
            velocity = torch.zeros(self.na, 2, device=self.device)
            for i in range(self.na):
                select_states[i, :] = state[i, i, :]
                velocity[i, :] = state[i, i, 2:4]
            velocity = velocity.reshape(-1).unsqueeze(0)
            state_h_mean = torch.clone(select_states).reshape(-1, self.observation_dim_per_agent)
        else:
            state_h_mean = torch.clone(state).reshape(-1, self.observation_dim_per_agent)

        # Laplacian
        laplacian = torch.kron(laplacian_base.unsqueeze(2), torch.ones((1, 1, self.observation_dim_per_agent), device=self.device))
        laplacian = laplacian.reshape(-1, self.na, self.observation_dim_per_agent)
        ...
        u_log_std = self.std_net(torch.cat((std_input.to(torch.float32), u_mean.reshape(-1, u_mean.shape[2]).unsqueeze(1).repeat(1, self.na, 1)), dim=2))
        u_log_std = torch.tanh(u_log_std)
        u_log_std = self.log_std_min + 0.5 * (self.log_std_max - self.log_std_min) * (u_log_std + 1)
...
def laplacian(q_agents, r_communication, na, device):
    Q1 = q_agents.repeat(1, na, 1)
    Q2 = torch.kron(q_agents.contiguous(), torch.ones((1, na, 1), device=device))
    Q = (Q1 - Q2).norm(p=2, dim=2).reshape(q_agents.shape[0], na, na)
    L = Q.le(r_communication).float()
    L = L * torch.sigmoid(-(2.0) * (Q - r_communication))
    return L
...
class PIMARL_actor(nn.Module):
    ...
    def __init__(self, actor_config):
        ...
        if self.scenario_name == "food_collection_vmas" or (self.scenario_name == "simple_spread_food" and not actor_config["preprocessor"]) or self.scenario_name == "simple_spread":
            ...
            self.observation_dim_per_agent = 8
        elif self.scenario_name == "navigation_robotarium":
            self.observation_dim_per_agent = 6
        else:
            self.observation_dim_per_agent = actor_config["observation_dim_per_agent"]
        self.action_dim_per_agent = actor_config["action_dim_per_agent"]
...
class Att_R(nn.Module):
    ...
    def forward(self, x, laplacian, scenario_name):
        ...
        if scenario_name == "simple_spread_food" or scenario_name == "grassland_vmas" or scenario_name == "adversarial_vmas" or scenario_name == "simple_spread" or scenario_name == "navigation_robotarium":
            x = (self.mlp_in(x.reshape(-1, self.input_dim)).reshape(x.shape[0], self.na, -1) *
