...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        for i in range(self.num_agents):
            agent = Agent(
                name=f"agent {i}",
                collide=True,
                shape=Sphere(radius=(0.08 if i < self.num_adversaries else 0.08)),
                color=(Color.RED if i < self.num_adversaries else Color.BLUE),
                adversary=(True if i < self.num_adversaries else False),
                max_speed=(3.0 if i < self.num_adversaries else 3.0),
                movable=True,
                u_range=4.0
            )
        ...
        if agent.collide:
            for a in self.world.agents:
                for b in self.world.agents:
                    if (a != agent and b != agent) and (a.adversary and not b.adversary):
                        killed = self.world.is_overlapping(a, agent) & self.world.is_overlapping(b, a) & (self.live[:, int(a.name[6:])] == 1.0)
                        pos_rew[killed] += 5

        if agent.collide:
            for a in self.world.agents:
                for b in self.world.agents:
                    if (a != agent and b != agent) and (a.adversary and b.adversary) and (a != b):
                        killed = self.world.is_overlapping(a, agent) & self.world.is_overlapping(b, agent) & (self.live[:, int(agent.name[6:])] == 1.0)
                        pos_rew[killed] -= 5
    ...
    def adversary_reward(self, agent: Agent):
        ...
        if agent.collide:
            for a in self.world.agents:
                for b in self.world.agents:
                    if (a != agent and b != agent) and (not a.adversary and not b.adversary) and (a != b):
                        killed = self.world.is_overlapping(a, agent) & self.world.is_overlapping(b, agent) & (self.live[:, int(agent.name[6:])] == 1.0)
                        pos_rew[killed] -= 5
                        self.live[killed, int(agent.name[6:])] = 0
                        if killed.shape[0] == 1 and killed[0] == True:
