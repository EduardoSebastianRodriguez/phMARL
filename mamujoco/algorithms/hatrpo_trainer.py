...
class HATRPO():
    ...
    def kl_divergence(self, obs, dynamics, rnn_states, action, masks, available_actions, active_masks, new_actor, old_actor):
        _, _, mu, std, probs = new_actor.evaluate_actions(obs, dynamics, rnn_states, action, masks, available_actions, active_masks)
        _, _, mu_old, std_old, probs_old = old_actor.evaluate_actions(obs, dynamics, rnn_states, action, masks, available_actions, active_masks)
        if mu.grad_fn==None:
    ...
    def conjugate_gradient(self, actor, obs, dynamics, rnn_states, action, masks, available_actions, active_masks, b, nsteps, residual_tol=1e-10):
        x = torch.zeros(b.size()).to(device=self.device)
        r = b.clone()
        p = b.clone()
        rdotr = torch.dot(r, r)
        for i in range(nsteps):
            _Avp = self.fisher_vector_product(actor, obs, dynamics, rnn_states, action, masks, available_actions, active_masks, p)
    ...
    def fisher_vector_product(self, actor, obs, dynamics, rnn_states, action, masks, available_actions, active_masks, p):
        p.detach()
        kl = self.kl_divergence(obs, dynamics, rnn_states, action, masks, available_actions, active_masks, new_actor=actor, old_actor=actor)
        kl = kl.mean()
    ...
    def trpo_update(self, sample, update_actor=True):
        ...
        """
        share_obs_batch, obs_batch, dynamics_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, \
