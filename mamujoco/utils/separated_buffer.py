...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, share_obs_space, act_space):
        ...
        self.dynamics = np.zeros((self.episode_length + 1, self.n_rollout_threads, 6), dtype=np.float32)
    ...
    def insert(self, share_obs, obs, dynamics, rnn_states, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None):
        self.share_obs[self.step + 1] = share_obs.copy()
        self.obs[self.step + 1] = obs.copy()
        self.dynamics[self.step + 1] = dynamics.copy()
    ...
    def chooseinsert(self, share_obs, obs, dynamics, rnn_states, rnn_states_critic, actions, action_log_probs,
                     value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None):
        self.share_obs[self.step] = share_obs.copy()
        self.obs[self.step] = obs.copy()
        self.dynamics[self.step] = dynamics.copy()
        self.rnn_states[self.step + 1] = rnn_states.copy()
        self.rnn_states_critic[self.step + 1] = rnn_states_critic.copy()
        self.actions[self.step] = actions.copy()
        self.action_log_probs[self.step] = action_log_probs.copy()
    ...
    def after_update(self):
        ...
        self.dynamics[0] = self.dynamics[-1].copy()
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        dynamics = self.dynamics[:-1].reshape(-1, *self.dynamics.shape[2:])
        ...
        for indices in sampler:
            ...
            dynamics_batch = dynamics[indices]
            ...
            if self.factor is None:
                yield share_obs_batch, obs_batch, dynamics_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch
            else:
                factor_batch = factor[indices]
                yield share_obs_batch, obs_batch, dynamics_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, factor_batch
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        for start_ind in range(0, n_rollout_threads, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                dynamics_batch.append(self.dynamics[:-1, ind])
            ...
            dynamics_batch = np.stack(dynamics_batch, 1)
            ...
            dynamics_batch = _flatten(T, N, dynamics_batch)
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                dynamics_batch.append(dynamics[ind:ind+data_chunk_length])
            ...
            dynamics_batch = np.stack(dynamics_batch)
            ...
            dynamics_batch = _flatten(L, N, dynamics_batch)
