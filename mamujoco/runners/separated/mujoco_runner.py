...
from functions import PIMARL_actor, PIMARL_qvalue
...
class MujocoRunner(Runner):
    ...
    def run(self):
            ...
            for episode in range(episodes):
                ...
                for step in range(self.episode_length):
                    ...
                    obs, share_obs, rewards, dones, infos, _, dynamics = self.envs.step(actions)
                    ...
                    data = obs, share_obs, rewards, dones, infos, \
                           ...
                           rnn_states, rnn_states_critic, dynamics
        ...
        for agent_id in range(self.num_agents):
            ...
            value, action, action_log_prob, rnn_state, rnn_state_critic \
                = self.trainer[agent_id].policy.get_actions(self.buffer[agent_id].share_obs[step],
                                                            ...
                                                            self.buffer[agent_id].dynamics[step],
    ...
    def insert(self, data):
        ...
        values, actions, action_log_probs, rnn_states, rnn_states_critic, dynamics = data
        ...
        for agent_id in range(self.num_agents):
            self.buffer[agent_id].insert(share_obs[:, agent_id], obs[:, agent_id], dynamics[:, 3:],
                                         rnn_states[:, agent_id],
    ...
    def eval(self, total_num_steps):
            ...
            dynamics = np.zeros((self.n_eval_rollout_threads, 6), dtype=np.float32)
            ...
            while True:
                ...
                eval_obs, eval_share_obs, eval_rewards, eval_dones, eval_infos, _, dynamics = self.eval_envs.step(
                    eval_actions)
                dynamics = dynamics[:, 3:]
                for eval_i in range(self.n_eval_rollout_threads):
                ...
                if eval_episode >= self.all_args.eval_episodes:
                    ...
                    np.save(str(self.run_dir)+"/eval_average_episode_rewards_"+str(total_num_steps)+".npy", np.mean(eval_episode_rewards))
                    break
