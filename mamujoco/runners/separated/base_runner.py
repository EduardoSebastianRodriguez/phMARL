...
from utils.separated_buffer import SeparatedReplayBuffer, ReplayBuffer
...
class Runner(object):
    def __init__(self, config):
        ...
        elif self.all_args.algorithm_name == "ph-MARL":
            dummy = 1
        else:
        ...
        self.policy = []
        if self.all_args.algorithm_name == "ph-MARL":
            dummy = 1
        else:
        ...
        for agent_id in torch.randperm(self.num_agents):
            ...
            if self.all_args.algorithm_name == "hatrpo":
                old_actions_logprob, _, _, _, _ =self.trainer[agent_id].policy.actor.evaluate_actions(self.buffer[agent_id].obs[:-1].reshape(-1, *self.buffer[agent_id].obs.shape[2:]),
                                                            self.buffer[agent_id].dynamics[:-1].reshape(-1, *self.buffer[agent_id].dynamics.shape[2:]),
